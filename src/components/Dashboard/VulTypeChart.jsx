...
function VulTypeChart() {
  return (
    <div className="bg-white dark:bg-slate-900 backdrop-blur-xl rounded-b-2xl p-6 border
     border-slate-200/50 dark:border-slate-700/50">
      <div className="mb-6">
        <h3 className="text-lg- font-bold text-slate-800 dark:text-white">Type</h3>
      <p className="text-sm text-slate-500 dark:text-slate-400">Vulnerabilities Type Distribution</p>
      </div>
      <div className="h-48">
      ...
      <div className="space-y-3">
        {data.map((item, index) => {
            return <div className="flex items-center justify-between" key={index}>
                ...
                <span className="text-sm text-slate-600 dark:text-slate-400">{item.type}
                </span>
